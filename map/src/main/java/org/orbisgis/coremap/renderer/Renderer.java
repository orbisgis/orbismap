...
/**
 * Renderer contains all the logic of the Symbology Encoding process based on
 * java Graphics2D. This is an abstract class and subclasses provided effectives
 * methods according to the rendering target (e.g. bitmap image, SVG, pdf, etc.)
 *
...
public abstract class Renderer {
    ...
    public int drawVector(Graphics2D g2, MapTransform mt, ILayer layer,
            IProgressMonitor pm) throws SQLException {
    ...
    private int drawStyle(Style style, Graphics2D g2, MapTransform mt, ILayer layer,
            IProgressMonitor pm, Envelope extent) throws SQLException {
        int layerCount = 0;
        LinkedList<Symbolizer> symbs = new LinkedList<Symbolizer>();
        ISpatialTable spatialTable = layer.getSpatialTable();
        ...
        geofilter.append("'").append(GF.toGeometry(extent).toText()).append("' :: GEOMETRY && ").append(geomColumnName);
         try {
            ...
            IProgressMonitor rulesProgress = pm.startTask(rList.size());
            for (Rule r : rList) {
                ...
                IProgressMonitor rowSetProgress = rulesProgress.startTask("Drawing " + layer.getName() + " (Rule " + r.getName() + ")", 1);
                ...
                while (spatialTableQuery.next()) {
                    ...
                    if (theGeom != null) {
                        ...
                        beginFeature(row, spatialTableQuery);
                        ...
                        endFeature(row, spatialTableQuery);
                    }
                    rowSetProgress.endTask();
                }
                //spatialTable.beforeFirst();
                endLayer(r.getName());
...
//                g2.drawString(ex.toString(), EXECP_POS, EXECP_POS);
     ...
     * @param lay Source of information
     * @param progressMonitor Progress monitor to report the status of the
     * drawing
     */
            ...
            if (pm.isCancelled()) {
                ...
                if (layer.isVisible() && extent.intersects(layer.getEnvelope())) {
                    try {
                        ...
                        LOGGER.error("Layer " + layer.getName() + " not drawn", e);
                    }
                }
            }
            pm.endTask();
     ...
     * @param pm
     */
    private void drawRaster(Graphics2D g2, MapTransform mt, ILayer layer, int width, int height, IProgressMonitor pm) throws SQLException {
        //TODO Raster with h2
