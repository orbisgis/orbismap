...
import org.orbisgis.coremap.renderer.featureStyle.fill.HaloDrawer;
...
import org.orbisgis.coremap.renderer.featureStyle.transform.TransformBuilder;
import org.orbisgis.coremap.renderer.featureStyle.utils.WellKnownNameUtils;
...
import org.orbisgis.style.graphic.MarkGraphicSource;
...
public class MarkGraphicDrawer implements IGraphicDrawer<MarkGraphic> {
    ...
    public void draw(JdbcSpatialTable sp, Graphics2D g2, MapTransform mapTransform, MarkGraphic styleNode, Map<String, Object> properties) throws ParameterException, SQLException {
        ...
        if (shp == null) {
            shp = WellKnownNameUtils.getShape(WellKnownName.CIRCLE, styleNode.getViewBox(), properties, mapTransform.getScaleDenominator(),
                    mapTransform.getDpi(), styleNode.getMarkIndex(), styleNode.getMimeType());
        }
     ...
     * @throws ParameterException
     */
    private MarkGraphicSource getSource(JdbcSpatialTable sp, Graphics2D g2, MapTransform mapTransform, MarkGraphic styleNode, Map<String, Object> properties) throws ParameterException {
        StringParameter wkn = styleNode.getWkn();
        VariableOnlineResource onlineResource = styleNode.getOnlineResource();
        if (wkn != null) {
            return WellKnownName.fromString(wkn.getValue(properties));
