...
public class HatchedFillDrawer implements IFillDrawer<HatchedFill> {
    ...
    public void draw(JdbcSpatialTable sp, Graphics2D g2, MapTransform mapTransform, HatchedFill styleNode, Map<String, Object> properties) throws ParameterException, SQLException {
        ...
        if (stroke != null) {
            ...
            try {
                double pDist;
                pDist = DEFAULT_PDIST;
                RealParameter distance = styleNode.getDistance();
                if (distance != null) {
                    pDist = UomUtils.toPixel(distance.getValue(properties), uom, mapTransform.getDpi(), mapTransform.getScaleDenominator(), null);
                }

                double alpha = DEFAULT_ALPHA;
                RealParameter angle = styleNode.getAngle();
                if (angle != null) {
                    alpha = angle.getValue(properties);
                }

                double hOffset = 0.0;
                RealParameter offset = styleNode.getOffset();
                if (offset != null) {
                    hOffset = UomUtils.toPixel(offset.getValue(properties), uom, mapTransform.getDpi(), mapTransform.getScaleDenominator(), null);
                }
    ...
    public static void drawHatch(JdbcSpatialTable sp, Graphics2D g2, Map<String,Object> properties, Shape shp,
        ...
        Double naturalLength = penStroke.getNaturalLength(properties, shp, mt);
