...
public class LocalizedTextTest {
        ...
        public void testLocaleSeparation() {
                ...
                assertTrue(
                        parts.length==1,"Invalid Locale parts size:"+parts.length);                
                parts = LocalizedText.separateLocale("en");
                assertTrue(
                        parts.length==1,"Invalid Locale parts size:"+parts.length);
                parts = LocalizedText.separateLocale("en-US");
                assertTrue(
                        parts.length==2,"Invalid Locale parts size:"+parts.length);
                parts = LocalizedText.separateLocale("en-US-x-twain");
                assertTrue(
                        parts.length==3,"Invalid Locale parts size:"+parts.length);
        }
        ...
        */
        ...
        public void testLocaleDeserialization() throws Exception {
                ...
                assertTrue(
                        locDeser.equals(loc),locDeser.toString()+" != "+loc.toString());
        }
