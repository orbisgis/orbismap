        ...
        while (!it.isDone()) {
            ...
            switch (type) {
                ...
                case PathIterator.SEG_CUBICTO:
                    LOGGER.warn("CUBIC TO {0}:{1}",coords[0],coords[1]);
                    break;
                case PathIterator.SEG_LINETO:
                    LOGGER.warn("LINE TO {0}:{1}",coords[0],coords[1]);
                    break;
                case PathIterator.SEG_MOVETO:
                    LOGGER.warn("MOVE TO {0}:{1}",coords[0],coords[1]);
                    break;
                case PathIterator.SEG_QUADTO:
                    LOGGER.warn("QUAD TO {0}:{1}",coords[0],coords[1]);
                    break;
                default:
                    LOGGER.warn("!DEFAULT!");
                    break;
    ...
    private static List<Shape> contourParallelShape(Shape shp, double offset) {
        ...
        for (List<Vertex> vertexes : shapes) {
            ...
            if (offsetVertexes.size() < 2) {
                LOGGER.error("Unable to compute perpendicular offset");
                return rawShapes;
            }
