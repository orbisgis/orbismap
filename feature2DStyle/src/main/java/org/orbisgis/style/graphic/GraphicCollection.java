...
public final class GraphicCollection extends StyleNode implements IUom {
     ...
     * @throws IOException 
     */
    public Rectangle2D getBounds(Map<String,Object> map, IMapTransform mt)
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                LOGGER.error("Error during graphic composition",ex);
            }
        }
     ...
     * @throws IOException
     */
    public void draw(Graphics2D g2, Map<String,Object> map, IMapTransform mt, AffineTransform at)
            throws ParameterException, IOException {
        for (Graphic g : graphics) {
            try {
                g.draw(g2, map, mt, at);
            } catch (ParameterException ex) {
                LOGGER.error("Could not render graphic",ex);
            }
        }
    }
