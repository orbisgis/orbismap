...
public class OnlineResource extends StyleNode implements ExternalGraphicSource, MarkGraphicSource {
     ...
     */
    public Rectangle2D.Double getJAIBounds(ViewBox viewBox, Map<String,Object> map, IMapTransform mt, String mimeType) throws ParameterException {
        try {
            if (rawImage == null) {
                rawImage = ImageIO.read(uri.toURL());
    ...
    public Rectangle2D.Double getSvgBounds(ViewBox viewBox, Map<String,Object> map,
            IMapTransform mt, String mimeType) throws ParameterException {
            /*
     ...
     * @param opacity
     */
    public void drawJAI(Graphics2D g2, AffineTransform at, IMapTransform mt, double opacity) {
        AffineTransform fat = new AffineTransform(at);
        double width = rawImage.getWidth();
        double height = rawImage.getHeight();
    ...
    @Override
    public void draw(Graphics2D g2, Map<String,Object> map,AffineTransform at, IMapTransform mt, double opacity, String mimeType) {
        if (mimeType != null && mimeType.equalsIgnoreCase("image/svg+xml")) {
     ...
     * @deprecated
     */
    public RenderedImage getSvgImage(ViewBox viewBox, Map<String,Object> map, IMapTransform mt, String mimeType)
