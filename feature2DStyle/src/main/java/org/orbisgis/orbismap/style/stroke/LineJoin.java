...
public enum LineJoin {
    ...
    /**
     * Build a new {@code LineJoin} from a {@code String token}.
     *
     * @param tok
     * @return A {@code LineJoin} value. Defaults to null if the name does not
     * match anything.
     */
    public static LineJoin fromString(String tok) {
        String token = tok == null ? "" : tok;
        if (token.equalsIgnoreCase("MITRE")) {
            return MITRE;
        } else if (token.equalsIgnoreCase("ROUND")) {
            return ROUND;
        } else if (token.equalsIgnoreCase("BEVEL")) {
            return BEVEL;
        }
        return null;
    }
