...
public enum LineCap {
    ...
    /**
     * Build a new {@code LineCap} from a {@code String token}.
     *
     * @param tok
     * @return A {@code LineCap} value. Defaults to null if the name does not
     * match anything.
     */
    public static LineCap fromString(String tok) {
        String token = tok == null ? "" : tok;
        if (token.equalsIgnoreCase("BUTT")) {
            return BUTT;
        } else if (token.equalsIgnoreCase("ROUND")) {
            return ROUND;
        } else if (token.equalsIgnoreCase("SQUARE")) {
            return SQUARE;
        }
        return null;
    }
