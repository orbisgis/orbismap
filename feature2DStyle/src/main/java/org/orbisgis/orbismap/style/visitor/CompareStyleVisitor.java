    ...
    protected void visitImpl(IStyleNode expectedStyleNode, IStyleNode actualStyleNode) throws ParameterException {
        ...
        if (expectedStyleNode instanceof Expression && actualStyleNode instanceof Expression) {
            try {
                ...
                if (!expectedParsed.toString().equalsIgnoreCase(actualParsed.toString())) {
                    throw new ParameterException("The styles doesn't have the same expression ; expected style node = " + expectedParsed + " and actual style node = " + actualParsed);
                }
