...
/**
 * Rule filter converter
 *
 */
public class RuleFilterConverter implements Converter {

    @Override
    public void marshal(Object value, HierarchicalStreamWriter writer, MarshallingContext marshallingContext) {
        String valuetoWrite = ((RuleFilter)value).getExpression();
        if (!valuetoWrite.isEmpty()) {
            writer.startNode(Feature2DStyleTerms.FILTER);
    ...
    public Object unmarshal(HierarchicalStreamReader reader, UnmarshallingContext unmarshallingContext) {
        return new RuleFilter(Feature2DStyleIO.unescapeXMLString(reader.getValue()));
    }

    @Override
    public boolean canConvert(Class type) {
        return type.equals(RuleFilter.class);
    }
}
